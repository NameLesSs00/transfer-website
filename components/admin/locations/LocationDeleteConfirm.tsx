...
export function LocationDeleteConfirm() {
  ...
  if (!isDeleteConfirmOpen || !locationPendingDelete) return null;

  async function handleDelete() {
    if (!locationPendingDelete) return;

    const result = await dispatch(removeLocation(locationPendingDelete.id));
    if (removeLocation.fulfilled.match(result)) {
      void dispatch(fetchLocations({ pageNumber, pageSize }));
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4">
      <section className="w-full max-w-md rounded-2xl bg-white p-6 shadow-[0_18px_70px_rgba(15,23,42,0.18)]">
        <h2 className="text-xl font-bold text-transfer-dark">Delete Location</h2>
        <p className="mt-2 text-sm font-medium leading-relaxed text-[#667085]">
          Delete {locationPendingDelete.name}? This will remove it from the locations list.
        </p>

        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
          <button
            onClick={() => dispatch(closeDeleteLocationConfirm())}
