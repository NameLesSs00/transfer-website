...
export function LocationsToolbar() {
  ...
  const { isDescending, pageSize, search, sortBy } = useAppSelector((state) => state.locations);
  const [searchValue, setSearchValue] = useState(search);

  function handleSearch(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    dispatch(setLocationsSearch(searchValue));
    void dispatch(fetchLocations({ pageNumber: 1, pageSize, search: searchValue }));
  }

  function handleSortToggle() {
    const nextIsDescending = !isDescending;
    dispatch(setLocationsSort({ sortBy: "name", isDescending: nextIsDescending }));
    void dispatch(
      fetchLocations({
        pageNumber: 1,
        pageSize,
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-gray-100 p-5 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <h2 className="text-xl font-bold text-transfer-dark">Locations</h2>
        <p className="mt-1 text-sm font-medium text-[#667085]">
          Manage pickup and drop-off places with exact map coordinates.
        </p>
      </div>
