...
export function VehiclesToolbar() {
  ...
  function handleSearch(event: FormEvent<HTMLFormElement>) {
    ...
    dispatch(setVehiclesSearch(searchValue));
    void dispatch(fetchVehicles({ pageNumber: 1, pageSize, search: searchValue }));
  }

  function handleSortToggle() {
    const nextIsDescending = !isDescending;
    dispatch(setVehiclesSort({ sortBy: "name", isDescending: nextIsDescending }));
    void dispatch(
      fetchVehicles({
        pageNumber: 1,
        pageSize,
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-gray-100 p-5 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <h2 className="text-xl font-bold text-transfer-dark">Vehicles</h2>
        <p className="mt-1 text-sm font-medium text-[#667085]">
          Manage fleet vehicles, assignments, image, capacity, and status.
        </p>
      </div>
