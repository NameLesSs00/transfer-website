...
export function VehicleCategoriesToolbar() {
  ...
  function handleSearch(event: FormEvent<HTMLFormElement>) {
    ...
    dispatch(setVehicleCategoriesSearch(searchValue));
    void dispatch(fetchVehicleCategories({ pageNumber: 1, pageSize, search: searchValue }));
  }

  function handleSortToggle() {
    const nextIsDescending = !isDescending;
    dispatch(setVehicleCategoriesSort({ sortBy: "name", isDescending: nextIsDescending }));
    void dispatch(
      fetchVehicleCategories({
        pageNumber: 1,
        pageSize,
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-gray-100 p-5 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <h2 className="text-xl font-bold text-transfer-dark">Vehicle Categories</h2>
        <p className="mt-1 text-sm font-medium text-[#667085]">
          Manage fleet category names, descriptions, and pricing behavior.
        </p>
      </div>
